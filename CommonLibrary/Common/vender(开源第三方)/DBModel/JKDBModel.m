    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                ...
                NSLog(@"%@:===%@",columeName,[resultSet stringForColumn:columeName]);
...
//    }];
    ...
    return [self findDdtaByCriteria:sql];
}

/** 通过条件查找数据 */
+ (NSArray *)findDdtaByCriteria:(NSString *)criteria
{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
//        NSString *tableName = NSStringFromClass(self.class);
        NSString *sql =criteria;
        FMResultSet *resultSet = [db executeQuery:sql];
        while ([resultSet next]) {
            JKDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
...
+ (int )getCountWithCriteria:(NSString *)criteria
{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    __block int temp =0;
    [jkDB.dbQueue inDatabase:^(FMDatabase * _Nonnull db) {
        NSString *tableName = NSStringFromClass(self.class);
        NSString *sql = [NSString stringWithFormat:@"SELECT COUNT(*) AS number FROM %@",tableName];
        NSLog(@"%@",sql);
        FMResultSet *resultSet = [db executeQuery:sql];
        
         while ([resultSet next]) {
            temp = [resultSet intForColumn:@"number"];
         }
    }];
    return temp;
}
